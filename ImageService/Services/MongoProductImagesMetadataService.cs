...
public class MongoProductImagesMetadataService : IProductImagesMetadataService
{
    ...
    public async Task<ProductImagesMetadata> GetProductImagesMetadataAsync(int productId)
    {
        ...
        return await _metadataCollection.Find(filter).FirstOrDefaultAsync() ?? new ProductImagesMetadata(productId, new List<string>(), new NoMetadataAvaiable());
    }
    ...
    public async Task UpdateMetadataAsync(ProductImagesMetadata productMetadata)
    {
        ...
        await _metadataCollection.FindOneAndReplaceAsync(x => x.ProductId == productMetadata.ProductId, productMetadata);
