...
public class AddressServiceTests : IClassFixture<DatabaseFixture>
{
  ...
  public AddressServiceTests(DatabaseFixture databaseFixture)
  {
	...
	_dbContext = new ApplicationDbContextFake(new DbContextOptionsBuilder<Data.ApplicationDbContext>()
	  .UseSqlServer(_databaseFixture.dbContainer.GetConnectionString()).Options);
  ...
  public async Task RemoveAddress_ExistentGuid_AmountOfUserAddressesDecremented()
  {
	...
	Assert.Equal(addressesCountBeforeDelete,
	  addressesCountAfterDelete + 1);
  }
