...
namespace OrderService.MessageQueue.Sagas.Activities
{
  ...
  ) : IStateMachineActivity<CancelOrderState, IOrderCancellationRequest>
  {
    ...
    public async Task Execute(BehaviorContext<CancelOrderState, IOrderCancellationRequest> context, IBehavior<CancelOrderState, IOrderCancellationRequest> next)
    {
    ...
    public Task Faulted<TException>(BehaviorExceptionContext<CancelOrderState, IOrderCancellationRequest, TException> context, IBehavior<CancelOrderState, IOrderCancellationRequest> next) where TException : Exception
    {
    ...
    private async Task OrderCancellationUnavailable(Order order, BehaviorContext<CancelOrderState, IOrderCancellationRequest> context)
    {
      await context.Publish<IOrderCancelledCancellationUnavailable>(new
      {
        context.Message.OrderId
      });
    ...
    private async Task CancelOrder(Order order, BehaviorContext<CancelOrderState, IOrderCancellationRequest> context)
    {
      order.Status = OrderStatus.Type.Cancelled;
      _db.Entry(order).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
      await _db.SaveChangesAsync(CancellationToken.None);
      await context.Publish(new OrderCancelledRemoveProductsReservationCommand()
      {
