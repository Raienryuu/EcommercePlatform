...
public class UserServiceTests
{
  ...
  public async Task RegisterNewUser_ValidData_ReturnsSuccess()
  {
    ...
    _userManagerMock
      .Setup(m => m.CreateAsync(It.IsAny<IdentityUser<Guid>>()))
      .ReturnsAsync(IdentityResult.Success)
      .Callback<IdentityUser<Guid>>(
        (user) =>
        {
    ...
    _userManagerMock
      .Setup(m => m.AddToRoleAsync(It.IsAny<IdentityUser<Guid>>(), It.IsAny<string>()))
      .ReturnsAsync(IdentityResult.Success);
  ...
  public async Task GetUsernameForLoggedUser_ValidUserId_ReturnsUsername()
  {
    ...
    Assert.True(result.IsSuccess);
    Assert.Equal(user.UserName, result.Value, true);
  }
  ...
  public async Task RegisterNewUser_CreateAsyncFails_ReturnsError()
  {
    ...
    _userManagerMock.Setup(m => m.CreateAsync(It.IsAny<IdentityUser<Guid>>())).ReturnsAsync(failedResult);
    var result = await _userService.RegisterNewUser(newUser, CancellationToken.None);
